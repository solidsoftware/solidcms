...
class Core_application extends Auth_controller{
	...
	public function index(){
		...
		$breadcrumb = array();
		$breadcrumb['active'] = 'Core Application';
		$content['breadcrumb'] = Modules::run('breadcrumb/top_breadcrumb', $breadcrumb);

		$panel =array(
				'menus' =>array(
					array('icon'=>'icon-plus-2','type'=>'link','link'=>$this->solid_dispatcher->get_url('core_application','core_application','input'),'label'=>'add'),
					array('icon'=>'icon-trashcan','type'=>'link','link'=>site_url(),'label'=>'delete'),
				)
		);
		$content['toppanel'] = Modules::run('panel_menu/table_panel',$panel);
		$content['messenger']=Modules::run('messenger') ;
		$content['paging']=Modules::run('paging') ;
		
		if(isset($_GET['comp']) && $_GET['comp']=='true')
